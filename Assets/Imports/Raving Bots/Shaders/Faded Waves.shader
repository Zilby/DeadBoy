...
Shader "Water/Faded Waves"
{
	...
	SubShader 
	{
		...
		Pass 
		{
			...
			{
                float original_x = IN.texcoord.x;
                ...
                float original_a = _Color.a;
                float depth_height = IN.texcoord.y;
                float depth_edges = min(1 - pow((abs(0.5f - original_x) * 2), 4), 1);
                _Color.a = original_a * depth_height * depth_edges;
